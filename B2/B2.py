...
def main():
    ...
    # load the train data
    train_data_path = "C:/Users/CJyM2/Desktop/AMLS_22-23_SN19002093/Datasets/cartoon_set/img/"
    train_label_path = "C:/Users/CJyM2/Desktop/AMLS_22-23_SN19002093/Datasets/cartoon_set/labels.csv"
    train_name2label = {}
    with open(train_label_path, "r") as read_file:
        ...
        for i in range(1, len(lines)):
            ...
            name = values[3]#serial number is in the fourth column
            eye = int(values[1])#labels of eye colours are in the second column
            train_name2label[name] = eye
    images = os.listdir(train_data_path)
    for file in images:
        path = os.path.join(train_data_path, file)
        img = custom_transform(Image.open(path))
        label = train_name2label[file]
        train_data.append(img)
        train_label.append(label)

    # load the test data
    test_data_path = "C:/Users/CJyM2/Desktop/AMLS_22-23_SN19002093/Datasets/cartoon_set_test/img/"
    test_label_path = "C:/Users/CJyM2/Desktop/AMLS_22-23_SN19002093/Datasets/cartoon_set_test/labels.csv"
    test_name2label = {}
    with open(test_label_path, "r") as read_file:
        ...
        for i in range(1, len(lines)):
            ...
            name = values[3]#serial number is in the fourth column
            eye = int(values[1])#labels of eye colours are in the second column
            test_name2label[name] = eye
    images = os.listdir(test_data_path)
    for file in images:
        path = os.path.join(test_data_path, file)
        img = custom_transform(Image.open(path))
        label = test_name2label[file]
    ...
    class CartoonDataset(Dataset):
        #Custom Dataset for loading Cartoon set
    ...
    train_dataset = CartoonDataset(train_data, train_label)
    test_dataset = CartoonDataset(test_data, test_label)
    ...
    class Net(torch.nn.Module):
        def __init__(self):
            ...
            self.mlp = torch.nn.Sequential(torch.nn.Linear(4* 64 * 64, 2048),
                torch.nn.ReLU(),
                torch.nn.BatchNorm1d(2048),
                torch.nn.Linear(2048, 768))
            hidden_size = 768
            vocab_size = 5  # num classes
